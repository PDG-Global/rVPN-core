...
//! Chooses which named server should handle a given target host.
//!
...
//!
//! The router is built once from the configured routing rules and consulted
//! per flow (CLI SOCKS5: `proxy_common::route_connection`) or per DNS answer
//! / packet destination (mobile Direct TUN).
...
impl Router {
    /// Build the router from parsed routing rules. `known_servers` must list
    /// every valid server name (including `"default"`). Fails if a rule
    /// references an unknown server, a CIDR won't parse, or a domain pattern
    /// is malformed.
    pub fn build(
        known_servers: &[&str],
        routing: &HashMap<String, RoutingRule>,
    ) -> Result<Self> {
        let known: std::collections::HashSet<&str> = known_servers.iter().copied().collect();
    ...
    /// Return the server name to use for a target host.
    ///
    /// `host` is a raw hostname or literal IP (no port, no brackets).
    /// Hostname patterns are tried first, then a CIDR lookup if `host`
    /// parses as an `IpAddr`. Falls back to `"default"` when nothing matches.
    ...
    /// Return the server name for a literal IP address (CIDR rules only;
    /// domain rules never match). Used by Direct TUN packet dispatch where
    /// only the destination IP is available. Falls back to `"default"`.
    pub fn choose_ip<'a>(&'a self, ip: IpAddr) -> &'a str {
        if let Some((_, name)) = self.ip_rules.longest_match(ip) {
            return name;
        }
        &self.default_name
...
mod tests {
    ...
    fn build(routing: &HashMap<String, RoutingRule>, known: &[&str]) -> Router {
        Router::build(known, routing).unwrap()
    }
    ...
    #[test]
    fn choose_ip_matches_cidr_only() {
        let mut r: HashMap<String, RoutingRule> = HashMap::new();
        r.insert("sg".into(), RoutingRule {
            domains: vec!["google.com".into()],
            ips: vec!["8.8.8.0/24".into()],
        });
        let router = build(&r, &["sg", DEFAULT_SERVER_NAME]);
        assert_eq!(router.choose_ip("8.8.8.8".parse().unwrap()), "sg");
        assert_eq!(
            router.choose_ip("9.9.9.9".parse().unwrap()),
            DEFAULT_SERVER_NAME
        );
    }
    ...
    fn unknown_server_rejected() {
        let mut r: HashMap<String, RoutingRule> = HashMap::new();
        r.insert("mars".into(), RoutingRule::default());
        assert!(Router::build(&["sg", DEFAULT_SERVER_NAME], &r).is_err());
    }

    #[test]
    fn routing_default_rejected() {
        let mut r: HashMap<String, RoutingRule> = HashMap::new();
        r.insert(DEFAULT_SERVER_NAME.into(), RoutingRule::default());
        assert!(Router::build(&[DEFAULT_SERVER_NAME], &r).is_err());
    }
}
